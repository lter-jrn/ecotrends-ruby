	...
	GEvent.addListener(marker, "mouseover", function() {
  	...
  	if (mapPage == "participating-sites") {
		...
		GEvent.addListener(marker, "click",	function() {
			golbalSite=site;
			var formtag="<form name='browseForm' action='/advancedBrowseServlet' method='post' target='_top' ><input type='hidden' name='browseValue' value='' />";
		    var endformtag="</form>";
		    
			formtag=formtag+html+endformtag;
			marker.openInfoWindowHtml(formtag);
...
function keywordSearch() {
    if(login==1)
    {
	document.browseForm.browseValue.value=golbalSite;
	document.browseForm.submit();
    }
    else
    	alert("please log in to access the ecotrends content");
    return true;
  }
...
function toggleMarkers() {
	...
	if (markersOn) {	
		...
		for (id in markers) {
			...
			markerSITE.push(markers[id].site);
...
{
	golbalSite=markerSITE[markerNum];
	var formtag="<form name='browseForm' action='/advancedBrowseServlet' method='post' target='_top' ><input type='hidden' name='browseValue' value='' />";
    var endformtag="</form>";
    
	formtag=formtag+markerHTML[markerNum]+endformtag;
	mapMarkers[markerNum].openInfoWindowHtml(formtag);
